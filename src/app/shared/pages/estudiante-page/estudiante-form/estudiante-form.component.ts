...
export class EstudianteFormComponent {
  ...
  async guardar() {
    ...
    if (this.accion == "add") {
      ...
      let result = await this.estudianteServicio.agregarStudent(estudiante)
      ...
      let result = await this.estudianteServicio.editarStudent(this.estudiante?.idestudiante!, estudiante);
      if (result.state == "success") {
