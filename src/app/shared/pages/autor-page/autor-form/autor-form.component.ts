...
import { IAutor, IAutorAdd } from '../autor.helpers';
...
export class AutorFormComponent {
  ...
  async guardarAutor() {
    ...
    if (this.accion == "add") {
      ...
      let result = await this.autorServicio.agregarAuthor(autor)
      ...
      let autor: IAutor = {
        ...
        nacionalidad: datos.nacionalidad!,
        sexo: datos.sexo!,
        fecha_nacimiento: datos.sexo!
      }
      let result = await this.autorServicio.editarAuthor(this.autor?.idautor!, autor);
      if (result.state == "successs") {
